...
import { TreeSetPositionConstraintAction, SetAspectRatioAction } from './actions';
 ...
 */
export function setGenerateTreeAction(nodes: KNode[], target: KNode, parent: KNode | undefined, event: MouseEvent) {
            ...
            if (event.pageX > lowX && event.pageX < highX
                && event.pageY > lowY && event.pageY < highY) {
                    ...
                    if (actualPosition !== target.properties.currentPosition && actualPosition !== -1) {
                        result = new TreeSetPositionConstraintAction(
